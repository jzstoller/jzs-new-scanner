...
export default class Logger {
  ...
  private async resolveLogFile(): Promise<TFile> {
    if (
      this.cachedFile &&
      this.app.vault.getAbstractFileByPath(this.cachedFile.path) === this.cachedFile
    ) {
    ...
    this.resolvingFile = (async () => {
      const existing = this.app.vault.getAbstractFileByPath(this.logFilePath);
      ...
      try {
        this.cachedFile = await this.app.vault.create(this.logFilePath, '');
        return this.cachedFile;
      } catch (err) {
        const retry = this.app.vault.getAbstractFileByPath(this.logFilePath);
  ...
  private async appendToLogFile(text: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Logger: failed to append to ${this.logFilePath}: ${String(err)}`);
    }
  }
